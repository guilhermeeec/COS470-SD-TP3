...
do
    ...
    resultado_output=$(python3 resultado_validator.py "resultado.txt" $n $r "test3_time_execution.txt")
    ...
    if [[ $resultado_output == "The result is valid." ]]; then
        ...
        if [[ $log_output == "Log file was successfully validated" ]]; then
            echo "Test 3 - n=$n: All validations done successfully."
        else
            echo "Test 3 - n=$n: Error: Log validation failed."
        fi
    else
        echo "Test 3 - n=$n: Error: Result validation failed."
    fi
